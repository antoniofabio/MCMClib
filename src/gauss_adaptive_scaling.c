...
static suff_t* suff_t_alloc(const size_t size, const size_t NB) {
...
static void suff_update(void* s, const gsl_vector* x) {
  ...
  /*TODO
    ...
    - if we finished a 'block', update the scaling factor and reset the AR counter
   */
...
mcmclib_amh* mcmclib_gauss_adaptive_scaling_alloc(gsl_rng* r, distrfun_p logdistr, void* logdistr_data,
  ...
  mcmclib_amh* ans = mcmclib_amh_alloc(mcmclib_mh_alloc(r, logdistr, logdistr_data, mh_q_alloc(r, x->size, scaling), x), N0,
				       suff_t_alloc(x->size, NB), suff_t_free, suff_update, gamma_update);
  return ans;
}
