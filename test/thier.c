...
gsl_vector *v_parent, *v_child, *x;
mcmclib_post_lpdf* lpost_data;

static double lprior(void* ignore, gsl_vector* x) {
  if((x0 < 1.0) && (x0 > 0.0))
    return 1.0;
  else
    return 0.0;
}
static double llik(void* ignore, gsl_vector* x) {
  return (x0 < gsl_vector_get(v_parent, 0)) ? 2.0 : 0.0;
}

static double lpost(double in_x) {
  gsl_vector_set(x, 0, in_x);
  return mcmclib_post_lpdf_compute(lpost_data, x);
}
