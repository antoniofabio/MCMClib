...
void Testmonitor2(CuTest* tc) {
  ...
  for(size_t i=0; i< p->Fn->size; i++)
    CuAssertTrue(tc, gsl_vector_get(p->Fn, i) == 0.5);

  gsl_vector_set_all(x, 0.5);
  mcmclib_monitor_ecdf_update(p, x);
  for(size_t i=0; i<6; i++)
    CuAssertTrue(tc, check_dequal(gsl_vector_get(p->Fn, i), 2.0 / 3.0));
  for(size_t i=6; i<10; i++)
    CuAssertTrue(tc, check_dequal(gsl_vector_get(p->Fn, i), 1.0 / 3.0));
