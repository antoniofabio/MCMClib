/**RAPT example 5: online mixture-based adaptive boundary*/
#include <stdio.h>
#include <memory.h>
...
#include <mvnorm.h>
#include <mixnorm.h>
/*basic RAPT algorithm*/
#include <rapt.h>
/*mixture fitting by OnLine EM*/
...
void target_distrib_init() {
  ...
  pi_target = mcmclib_mixnorm_lpdf_alloc(&(beta_vv.vector), pi);
}
void target_distrib_free() {
  for(int k=0; k<K; k++) {
    gsl_vector_free(mu[k]);
    gsl_matrix_free(Sigma[k]);
    mcmclib_mvnorm_lpdf_free(pi[k]);
  }
  mcmclib_mixnorm_lpdf_free(pi_target);
}
...
#define scanfif(i, fmt, target) if(argc > (i)) sscanf(argv[(i)], fmt, target)
  ...
  mcmclib_mixem_online* em = mcmclib_mixem_online_alloc(mu_hat, Sigma_hat, beta_hat, 0.5, T0);
  ...
  mcmclib_rapt* sampler =
    mcmclib_rapt_alloc(r, /*RNG*/
		       mcmclib_mixnorm_lpdf_compute, pi_target, /*target distribution*/
  ...
  for(int n=0; n<N; n++) {
    ...
    /*update online em fit*/
    mcmclib_mixem_online_update(em, x);
    save_gamma_hat(); /*store current parameters estimates*/
    /*update rapt proposal variances*/
    if((n>=T0)) {
      for(int k=0; k<K; k++) {
	gsl_matrix_memcpy(sampler->sigma_local[k], Sigma_hat[k]);
	gsl_matrix_scale(sampler->sigma_local[k], SCALING_FACTOR);
      }
    }
